...
namespace Interface {
    struct NodeProperties {
        static int count;
        std::string name = "node" + to_string(NodeProperties::count);
        // Placement
    ...
    struct ShapeProperties {
        static int count;
        std::string name = "shape" + to_string(ShapeProperties::count);
        // Appearance
        glm::vec3 albedo = glm::vec3(0.5);
        float roughness = 0.5;
        float metallic = 0;
        map<Tex_Type, std::string> textures;
        Shape_Surface surfacetype;
    ...
    struct LightProperties {
        static int count;
        std::string name = "light" + to_string(LightProperties::count);
        // Placement
        glm::vec3 position = glm::vec3(0);
    ...
    void ShowInterface() {
        ...
        static string _textureSelectionStr = "";
        // New light options
        ...
        static string _nodeTemplateSelectionStr = "";
        static shared_ptr<Drawable> _nodeTemplate;
        // Skybox options
        static string _skyboxSelectionStr = "";
        ...
        if (ImGui::CollapsingHeader("Assets", ImGuiTreeNodeFlags_DefaultOpen)) {
            if (ImGui::TreeNode("Textures")) {
                for (auto it = Resources::Textures.begin(); it != Resources::Textures.end(); it++) {
                    shared_ptr<Tex> texture = it->second;
                    if (ImGui::TreeNode(texture->Name.c_str())) {
                        if (ImGui::InputText(("Name##"+to_string(i++)).c_str(), &texture->NameTemp, ImGuiInputTextFlags_EnterReturnsTrue))
                            texture->Name = texture->NameTemp;
                        string format;
                        if (texture->Target == GL_TEXTURE_2D) {
                            ImGui::Text("Type: %s, %s", "2D", TexTypeToString(texture->Type).c_str());
                            ImGui::Text("Path: %s", texture->Paths[0].c_str());
                        } else if (texture->Target == GL_TEXTURE_CUBE_MAP) {
                            ImGui::Text("Type: %s, %s", "cubemap", TexTypeToString(texture->Type).c_str());
                            ImGui::Text("Paths:");
                            ImGui::Indent();
            ...
            if (ImGui::TreeNode("Models")) {
                for (auto it = Resources::Models.begin(); it != Resources::Models.end(); it++) {
                    shared_ptr<Model> model = it->second;
                    if (ImGui::TreeNode(model->Name.c_str())) {
                        if (ImGui::InputText(("Name##"+to_string(i++)).c_str(), &model->NameTemp, ImGuiInputTextFlags_EnterReturnsTrue))
        ...
        if (ImGui::CollapsingHeader("Shape Templates", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (_showNewShapeChild) {
                if (ImGui::BeginChild("New Shape Template", ImVec2(0, 0), ImGuiChildFlags_Border | ImGuiChildFlags_AlwaysAutoResize | ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY)) {
                    ImGui::InputText(("Template Name##"+to_string(i++)).c_str(), &_shapeProps.name, ImGuiInputTextFlags_EnterReturnsTrue);
                    ...
                    if (ImGui::TreeNode("Appearance")) {
                        ...
                        ImGui::RadioButton("Colored", &surface, SHAPE_SURFACE_COLOR); ImGui::SameLine();
                        ImGui::RadioButton("Textured", &surface, SHAPE_SURFACE_TEXTURE); ImGui::SameLine();
                        ImGui::RadioButton("Hybrid", &surface, SHAPE_SURFACE_HYBRID);
                        _shapeProps.surfacetype = Shape_Surface(surface);
                        if (_shapeProps.surfacetype == SHAPE_SURFACE_COLOR || _shapeProps.surfacetype == SHAPE_SURFACE_HYBRID) {
                            ImGui::ColorEdit3(("Albedo##"+to_string(i++)).c_str(), glm::value_ptr(_shapeProps.albedo), ImGuiColorEditFlags_Float | ImGuiColorEditFlags_HDR);
                            ImGui::SliderFloat(("Metallic##"+to_string(i)).c_str(), &_shapeProps.metallic, 0.0f, 1.0f);
                            ImGui::SliderFloat(("Roughness##"+to_string(i)).c_str(), &_shapeProps.roughness, 0.0f, 1.0f);
                        }
                        if (_shapeProps.surfacetype == SHAPE_SURFACE_TEXTURE || _shapeProps.surfacetype == SHAPE_SURFACE_HYBRID) {
                            for (int i = 0; i < 8; i++) {
                                Tex_Type textype = Tex_Type(i);
                                string label = TexTypeToString(textype) + " map";
                                string preview = _shapeProps.textures.count(textype) ? _shapeProps.textures[textype] : "";
            ...
            if (ImGui::TreeNode("Shape Templates##1")) {
                for (auto it = Resources::Shapes.begin(); it != Resources::Shapes.end(); it++) {
                    shared_ptr<Shape> shape = it->second;
                    if (ImGui::TreeNode(shape->Name.c_str())) {
                        if (ImGui::InputText(("Node Name##"+to_string(i++)).c_str(), &shape->NameTemp, ImGuiInputTextFlags_EnterReturnsTrue))
                        ...
                        if (ImGui::TreeNode("Appearance")) {
                            ...
                            ImGui::RadioButton("Textured", &surface, SHAPE_SURFACE_TEXTURE); ImGui::SameLine();
                            ImGui::RadioButton("Hybrid", &surface, SHAPE_SURFACE_HYBRID);
                            shape->SetSurfaceType(Shape_Surface(surface));
                            if (shape->GetSurfaceType() == SHAPE_SURFACE_COLOR || shape->GetSurfaceType() == SHAPE_SURFACE_HYBRID) {
                                ImGui::ColorEdit3(("Albedo##"+to_string(i++)).c_str(), glm::value_ptr(*shape->GetColorPtr()), ImGuiColorEditFlags_Float | ImGuiColorEditFlags_HDR);
                                ImGui::SliderFloat(("Metallic##"+to_string(i)).c_str(), shape->GetMetallicPtr(), 0.0f, 1.0f);
                                ImGui::SliderFloat(("Roughness##"+to_string(i)).c_str(), shape->GetRoughnessPtr(), 0.0f, 1.0f);
                            }
                            if (shape->GetSurfaceType() == SHAPE_SURFACE_TEXTURE || shape->GetSurfaceType() == SHAPE_SURFACE_HYBRID) {
                                for (int i = 0; i < 8; i++) {
                                    Tex_Type textype = Tex_Type(i);
                                    string label = TexTypeToString(textype) + " map";
                                    string preview = shape->Textures.count(textype) ? shape->Textures[textype]->Name : "";
                                    if (ImGui::BeginCombo(label.c_str(), preview.c_str())) {
        ...
        if (ImGui::CollapsingHeader("Scene", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (_showNewLightChild) {
                if (ImGui::BeginChild("New Light", ImVec2(0, 0), ImGuiChildFlags_Border | ImGuiChildFlags_AlwaysAutoResize | ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY)) {
                    ImGui::InputText("Name", &_lightProps.name, ImGuiInputTextFlags_AutoSelectAll);
                    ...
                    switch (_lightSelection) {
                        ...
                        case 1:
                            ...
                            ImGui::DragFloat3(("Size##"+to_string(i++)).c_str(), glm::value_ptr(_lightProps.size), 0.01, 0.0f, 0.0f, "%.2f");
                            break;
                        case 2: 
                            ImGui::Text("(future)");
                            ImGui::DragFloat3("Direction", glm::value_ptr(_lightProps.direction), 0.01, 0.0f, 0.0f, "%.2f");
                    ...
                    if (ImGui::Button("Create Light")) {
                        switch (_lightSelection) {
                            case 0: {
                                shared_ptr<DirectionalLight> dl = MainScene->CreateDirectionalLight();
                            ...
                            case 1: {
                                shared_ptr<PointLight> pl = MainScene->CreatePointLight();
            ...
            if (_showNewNodeChild) {
                if (ImGui::BeginChild("New Node", ImVec2(0, 0), ImGuiChildFlags_Border | ImGuiChildFlags_AlwaysAutoResize | ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY)) {
                    ImGui::InputText("Name", &_nodeProps.name, ImGuiInputTextFlags_AutoSelectAll);
                    ...
                    if (ImGui::Button("Create Node") && _nodeTemplate) {
                        shared_ptr<SceneNode> node = MainScene->CreateNode(_nodeTemplate);
            ...
            if (ImGui::TreeNode("Skybox")) {
                string preview = MainScene->EnvironmentMap->Name;
                if (ImGui::BeginCombo("Diffuse Texture", preview.c_str())) {
                    for (auto it = Resources::Textures.begin(); it != Resources::Textures.end(); it++) {
                        if (it->second->Type != TEX_DIFFUSE)
                            continue;
            ...
            if (ImGui::TreeNode("Lights")) {
                for (auto it = MainScene->DirLights.begin(); it != MainScene->DirLights.end(); it++) {
                    shared_ptr<DirectionalLight> dirlight = *it;
                    if (ImGui::TreeNode(dirlight->Name.c_str())) {
                        if (ImGui::InputText(("Name##"+to_string(i++)).c_str(), &dirlight->NameTemp, ImGuiInputTextFlags_EnterReturnsTrue))
                            dirlight->Name = dirlight->NameTemp;
                        ImGui::ColorEdit3(("Color##"+to_string(i++)).c_str(), glm::value_ptr(dirlight->Color), ImGuiColorEditFlags_Float | ImGuiColorEditFlags_HDR);
                        ImGui::DragFloat3(("Direction##"+to_string(i++)).c_str(), glm::value_ptr(dirlight->Direction), 0.01, 0.0f, 0.0f, "%.2f");
                        bool hasShadows = dirlight->HasShadows();
                        ImGui::Checkbox(("Cast Shadows##"+to_string(i++)).c_str(), &hasShadows);
                ...
                for (auto it = MainScene->PointLights.begin(); it != MainScene->PointLights.end(); it++) {
                    shared_ptr<PointLight> pointlight = *it;
                    if (ImGui::TreeNode(pointlight->Name.c_str())) {
                        if (ImGui::InputText(("Name##"+to_string(i++)).c_str(), &pointlight->NameTemp, ImGuiInputTextFlags_EnterReturnsTrue))
                            pointlight->Name = pointlight->NameTemp;
                        ImGui::ColorEdit3(("Color##"+to_string(i++)).c_str(), glm::value_ptr(pointlight->Color), ImGuiColorEditFlags_Float | ImGuiColorEditFlags_HDR);
                        ImGui::DragFloat3(("Position##"+to_string(i++)).c_str(), glm::value_ptr(pointlight->Position), 0.01, 0.0f, 0.0f, "%.2f");
                        ImGui::DragFloat3(("Size##"+to_string(i++)).c_str(), glm::value_ptr(pointlight->Size), 0.01, 0.0f, 0.0f, "%.2f");
            ...
            if (ImGui::TreeNode("Nodes")) {
                for (auto it = MainScene->Nodes.begin(); it != MainScene->Nodes.end(); it++) {
                    ...
                    if (ImGui::TreeNode(node->Name.c_str())) {
                        if (ImGui::InputText(("Name##"+to_string(i++)).c_str(), &node->NameTemp, ImGuiInputTextFlags_EnterReturnsTrue))
